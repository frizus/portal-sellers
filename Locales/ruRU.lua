...
L["bliz_options_tracked_message_lifetime_tooltip"] = "Сколько времени найденное сообщение выводится в трекере (по умолчанию %d |4секунда:секунды:секунд;)"
...
L["bliz_options_tracker_hide_similar_messages_tooltip"] = "Трекер группирует сообщения от одного игрока по одинаковым ключевым словам. Если включить, то только последнее сообщение группы будет сохраняться (по умолчанию %s)"
...
L["bliz_options_bottom"] = "Подсказка: поддерживается фильтрация по классу игрока и по искомым словам в сообщении.\n\n" ..
    "При вводе искомых слов поддерживаются операторы, например:\n" ..
