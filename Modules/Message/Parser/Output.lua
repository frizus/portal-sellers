...
function MessageParser:GetFormattedMessage(highlighted)
    ...
    for _, c in pairs(chars) do
        if type(c) ~= "table" then
            ...
            if c[1] == self.colorBegin or c[1] == self.highlightBegin then
                s = s .. "|c" .. c[2]
            elseif c[1] == self.colorEnd or c[1] == self.highlightEnd then
                s = s .. "|r"
...
function MessageParser:GetUnescapedMessage(highlighted)
    local s = ""
    local chars = highlighted and self.highlighted or self.chars
    for _, c in pairs(chars) do
        if type(c) ~= "table" then
            ...
            if c[1] == self.highlightBegin then
                s = s .. "|c" .. c[2]
            elseif c[1] == self.highlightEnd then
                s = s .. "|r"
            elseif c[1] == self.texture then
                s = s .. "|T" .. c[2] .. "|t"
            elseif c[1] == self.raidTag then
